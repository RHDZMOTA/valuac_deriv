...
precios.temporal <- matrix(, nrow = nrow(precios), ncol = ncol(precios)+1)
...
medias_rend <- s0 * exp(mean_rendi*time) 
sigmas_rend  <- sqrt(s0 ^ 2 * exp(2*mean_rendi*time) * (exp(time * sd_rendi ^ 2) - 1))

medias_precios <- matrix( , nrow = nrow(precios), ncol = ncol(precios)+1)
sigmas_precios <- matrix( , nrow = nrow(precios), ncol = ncol(precios)+1)
...
medias_precios[, 2] <- s0 * exp(i * medias_rend)
sigmas_precios[, 2] <- sqrt(s0^2 * exp(2 * i * medias_rend) *(exp(time*sigmas_rend^2)-1))
...
for(k in 1:3){
  sigma_pos <- as.data.frame(medias_precios[, 2] + k*sigmas_precios[, 2])
  sigma_neg <- as.data.frame(medias_precios[, 2] - k*sigmas_precios[, 2])
