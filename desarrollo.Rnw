...
\chapter{Obtención de datos}

Iniciando con el procedimiento, se obtienen los datos de la página \emph{Quandl}. Se muestran además gráficas del tipo de cambio desde un año atrás hasta el día actual. 
...
<<funcion_q, message=FALSE, warning=FALSE>>=
...
Como se explicaba en el documento teórico, se procede entonces a la generación del \emph{kernel} sumando las distribuciones de probabilidad para cada uno de los nuevos datos obteniendo así una nueva gráfica de densidad que se asemeja a la empírica mencionada anteriormente \protect\subref{signal_model}.

\chapter{Creación de simulaciones para el tipo de cambio}

En una segunda etapa se realizarán \Sexpr{deseos} simulaciones correspondientes para el plazo indicado que, en este caso, serán de \Sexpr{days} (pedidos por los proveedores). Previo a esto, se realizó la conversión de rendimientos a precios dado  que la variable de atención es el tipo de cambio USD/MXN.Después de la generación de las primeras \Sexpr{deseos} estimaciones, se procede a calcular un nuevo dato para cada día en cuestión, por lo tanto se tendrán \Sexpr{deseos} escenarios posibles para el tipo de cambio después de \Sexpr{days} días.\par
...
#Generación de simulaciones (trayectorias)
#El primer paso de las trayectorias depende del último dato real.
# Los demás dependen del simulado anterior.
...
\chapter{Resultados gráficos de trayectorias simuladas}

<<graficas, warning=FALSE, message=FALSE>>=
...
ggplot()+
  ...
  labs(title = 'Trayectorias simuladas a 90 días del tipo de cambio', 
       x = 'Días', y = 'USD / MXN')+ 
  theme(legend.position="none")
@
...
\begin{enumerate}
  \item \emph{Black-Scholes}: $C_t = S_t N(d_1) - ke^{-r(T-t)} N(d_2)$ donde:
    \begin{itemize}
      ...
      \item $N(x)$ es la función de distribución acumulada normal.
    \end{itemize}
  
  \item Prima con los datos obtenidos mediante la simulación calculada de la siguiente manera: $max(\hat{S_T} - k, 0)$ traído a valor presente.  
...
Para la elección de la prima en el cálculo de las ganancias fue tomada la correspondiente a \emph{Black-Scholes for exchange rates} de entre las tres antes mencionadas.
...
<<funcion_ganancias, message=FALSE, warning=FALSE>>=
...
ggplot()+
  ...
  labs(title = 'Trayectorias simuladas a 90 días con un precio strike',
       x = 'Días', y = 'USD / MXN')+ 
  theme(legend.position="none")

concepto1 <- c('Black-Scholes', 'Simulación', 'Black-Scholes foreign exchange rate')
...
En este apartado se procede a la creación de la función de distribución de las ganancias considerando la prima correspondiente (en el caso de la opción). 
<<histo_ganancias_opcion, warning=FALSE, message=FALSE>>=
#hist(ganan)
...
A continuación, se presenta el procedimiento equivalente anterior para el caso del contrato \emph{forward}
<<histo_ganan_forward, warning=FALSE, message=FALSE>>=
#hist(ganan)
...
En el caso de ejercer la opción, debe de considerarse el costo que implica la prima de la misma. A continuación, se procede a realizar el código que crea la distribución correspondiente junto con sus gráficas. 
<<ganancia_neta_opcion, message=FALSE, warning=FALSE>>=
...
ggplot(gggn)+geom_bar(aes(gggn$punt_mn,gggn$freq_relfn), stat="identity",width=deltan,
                      fill = I('dark green'),color = I('white'))+
  labs(title='Histograma de ganancias - opción neta',
...
concepto <- c('Forward', 'Opción Call', "Neto de la opción call", "Ahorro implícito")
diferencia_primas <- ct_rdrl - ct_tc
source("libreta.R")
@
Se presenta la valuación de opciones, se muestra una tabla comparativa de las distintas primas ya descritas anteriormente exceptuando la de las simulaciones pues se calculará en párrafos posteriores.
...
% \resizebox{\textwidth}{!}{%
  ...
  \Sexpr{concepto2[2]}&  \Sexpr{ct_rdrl_lib}    \\ \hline
...
A continuación, se presentan la comparación de la ganancia esperada y probabilidad de ganar 
\begin{table}[h!]
 % \resizebox{\textwidth}{!}{%
  \begin{tabular}{|l|l|l|}\hline
