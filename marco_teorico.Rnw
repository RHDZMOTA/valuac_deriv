...
Se propone la función $q(x)$ como el histograma de frecuencia, tomado como una función por partes. Se utiliza el método de la inversa para generar $n$ números aleatorios cuya distribución se asemeje a $q(x)$. 

Para relacionar la simulación con las cadenas de Markov, entra el factor de corrección $\alpha$ antes mencionado, donde dependiendo  si se satisface o no la condición, el dato simulado será el mismo del día anterior o bien el nuevo estimado. A partir de estos datos aleatorios, se realizan las simulaciones de las trayectorias (tomando en cuenta el dato inmediato anterior del tipo de cambio). Para la primera estimación se toma en cuenta el último dato real extraído, mientras que para los siguientes días se toma la estimación del día anterior.

\section{Derivados Financieros}

\textbf{Opción Call}\par
Una opción tipo call es un derivado financiero que otorga, al poseedor de la misma, el derecho a comprar el activo subyacente a un precio de ejercicio ($K$) antes definido, en una fecha de vencimiento determinada. Debido a que se tiene la opción de comprar, este derivado se ejercerá cuando el precio strike ($K$) esté por debajo del precio del mercado ($S_T$), ya que esto implica que se compra a un precio inferior al del mercado y hay un beneficio.

\textbf{Contrato Forward}\par
Un contrato forward es un contrato a plazo en el que dos partes tienen la obligación de comprar/vender el activo subyacente en el tiempo de vencimiento y a un precio de ejercicio ($K$) antes establecido.
